...
// Sunucu Bilgileri (Gerekiyorsa host kısmına sayısal IP yaz)
const SERVER_HOST = 'dracodan.funserver.top'; 
    ...
    <html>
      ...
      <body>
        <div class="card">
          ...
          <p>Adres: <b>${SERVER_HOST}:${SERVER_PORT}</b></p>
...
function startBot() {
  ...
  bot = mineflayer.createBot({
    ...
    port: SERVER_PORT,
    username: BOT_NAME,
    version: false // Sunucu sürümünü otomatik tespit eder
  });
...
app.post('/komut', (req, res) => { 
  if (bot && req.body.command) {
    ...
    addLog('[SİSTEM] Bot bağlı değil!');
  }
  res.redirect('/'); 
});

app.listen(PORT, () => console.log('Panel aktif.'));
